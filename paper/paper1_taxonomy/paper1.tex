...
Key files:
\begin{itemize}[nosep]
  \item \texttt{experiments/aitl\_blind\_nas/data\_loader.py}: Cover Type dataset logic
  \item \texttt{experiments/aitl\_blind\_nas/agent.py}: LLM agent with pivot \& termination rules
  \item \texttt{experiments/aitl\_blind\_nas/trainer.py}: Sandboxed code execution
  \item \texttt{experiments/aitl\_blind\_nas/runner.py}: AITL orchestration loop
\end{itemize}
...
cd Autopilot-zero-to-hero/experiments/aitl_blind_nas
pip install torch torchvision scikit-learn matplotlib numpy openai

# Cloud Agent experiment (OpenAI)
# Set OPENAI_API_KEY, then edit the MODEL constant at the top of
# runner.py (default: gpt-4o-mini), and run:
python runner.py

# Local Agent experiment (llama.cpp / Ollama)
# Point BASE_URL and MODEL in runner.py at your local server
# (see the file header for the exact configuration block), then run:
python runner.py
\end{lstlisting}

\subsection{Full Experiment Log}

The resulting loss curve for each run is written to \texttt{experiments/aitl\_blind\_nas/results/} when the script completes.
